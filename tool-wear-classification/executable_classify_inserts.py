...
Completed by: Mbaye DIONGUE (mbaye.diongue@etu.emse.fr)
...
def calculate_cost_LogReg(y, y_hat):
    ...
    #if y_hat<0:
    #    cost_i=-y*np.log(-y_hat)
    #elif y_hat>1:
    #    cost_i=(1-y*np.log(y_hat-1)
    #else:
...
seed(1235) # Pour la reproductibilité
...
if method==0:
    ...
    # -------------------SVM------------------------------------ -----------
    clf = svm.SVC(kernel="rbf", random_state=1234, gamma=0.03, C=0.01, probability=True)
    clf.fit(X_train, Y_train)
    Y_test_hat=clf.predict_proba(X_test)
    Y_test_hat= Y_test_hat[:,1]
    classifier=" SVM "
    ...
    # build a model
    model = Sequential()
    model.add(Dense(1, input_shape=(X_train.shape[1],), activation='sigmoid')) # Add an input shape! (features,)
    ...
    #------------------ Random Forest --------------------------------
    clf=rf( max_depth=3, random_state=0, criterion='entropy')
    clf.fit(X_train, Y_train)
    Y_test_hat=clf.predict_proba(X_test)
...
# the class "0" or "1". Thus we calculate the best threhold to asign class
...
# The best threhold is given by the point of the ROC curve that is the
# nearest to the point in the UP-left corner with coodinate (0,1)
best_threhold=0.4 #threholds[np.argmin((1 - test_tpr) ** 2 + test_fpr ** 2)]
...
print("***************")
print("The accuracy of the"+classifier+"classifier is {}".format(np.round(accuracy_,3)))
print("***************")
...
print("***************")
print("The F1-score of the"+classifier+"classifier is {}".format(np.round(f_score,3)))
print("***************")
# ============================================================
